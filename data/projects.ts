...
export type Project = {
  ...
  // Layout preference
  layout?: 'standard' | 'minimal' | 'lucidpast' | 'aam' | 'meleth-archive' | 'liquidread';
};
...
function ensureProject(p: any): Project {
  const base = {
    ...
    // Layout
    layout: (p.layout === 'standard' || p.layout === 'minimal' || p.layout === 'lucidpast' || p.layout === 'aam' || p.layout === 'meleth-archive' || p.layout === 'liquidread') ? p.layout : undefined,
  } as const;
