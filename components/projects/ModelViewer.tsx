/**
 * ModelViewer — shared component for displaying .glb 3D models.
 * Lazy-initialises the R3F canvas only when scrolled into view.
 * Uses frameloop="demand" for page performance (no continuous render loop).
 * Wraps canvas in ErrorBoundary + WebGL detection for safe fallback.
 *
 * Usage:
 *   <ModelViewer src="/path/to/model.glb" height={400} />
 */
...
function LoadingState({ label }: { label: string }) {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-3 bg-transparent">
      ...
      <span className="text-[#F5F0E8]/40 text-[9px] font-sans uppercase tracking-widest">{label}</span>
    </div>
  );
}
...
export default function ModelViewer({
  ...
  // Only initialise the canvas once the container has been scrolled into view.
  // After first visible, stays mounted (avoids re-init on scroll back).
  useEffect(() => {
  ...
  // Show loaded state after a short delay to let the model settle.
  useEffect(() => {
    if (!isVisible) return;
  ...
  return (
    <ErrorBoundary fallback={<UnavailableFallback height={height} />}>
      <div
        ...
        style={{ height }}
        aria-label={`Interactive 3D model viewer. ${label}`}
      >
        {/* Canvas — only mounts after scroll into view */}
        {isVisible && (
          <div className="absolute inset-0">
            <ModelViewerCanvas src={src} />
          </div>
        )}

        {/* Pre-load placeholder shown before visible */}
        {!isVisible && <LoadingState label="Scroll to load model" />}

        {/* Interaction hint — fades in after load */}
        <div
